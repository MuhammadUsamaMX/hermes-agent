...
def load_catalog_live() -> List[PluginCatalogEntry]:
    ...
    When the in-tree catalog is newer than the live doc, entries present in-tree but absent from
    the live list are appended — a stale mirror must not drop a freshly-merged catalog entry."""
    ...
    result = [in_tree[e.name] if e.name in in_tree and _prefer_in_tree_entry(in_tree[e.name], e, tree_is_newer) else e
              for e in entries]
    # Fix #119340: when the in-tree catalog is newer, surface entries the stale live doc
    # has not yet picked up (newly-merged additions the docs-site mirror has not regen'd).
    if tree_is_newer:
        for name, entry in in_tree.items():
